...
Medical commands preserve original clinical text, including Hebrew, while omitting private document routing and signatures from structured records. This is not de-identified. The filter is the one in `@maccabi/core`, shared with the MCP server, so both surfaces print the same fields for the same read. Keep redirected output private and out of Git.
...
- The handoff URL is a credential: it embeds the member id, its code and the member's `checksum_id`, which is per-member and authorises every study the member has. It is built inside the library, used once, and never printed, logged or returned.
