...
`login --id DIGITS` starts the same challenge without a terminal and sends one SMS; `login --code DIGITS` finishes it, as a second command. When several SMS numbers are available, `--id` alone prints them with option numbers and exits 3 without sending anything; repeat it with `--phone N`; a number that ID does not offer fails with `INVALID_PHONE_CHOICE`, which names the ones it does, and still sends nothing. `MACCABI_ID` and `MACCABI_OTP` are read only when the matching flag is absent, and passing both `--id` and `--code` at once is a usage error. These values are visible to other users of the machine through `ps` and are kept in shell history; the masked prompts are not. `login --status` reports `signed-in`, `pending-login`, or `signed-out` without touching the network. Every login path accepts `--json`.
...
**A finished login already does this.** `maccabi login` (and `maccabi login --code`) starts a detached `keep-alive --interval 240 --duration 3600` and returns. That holds off the idle timeout until the absolute cap, about an hour after login. `--no-keep-alive` skips it. `logout` stops the process so it cannot write the session file back.

Maccabi runs two separate clocks on a session. One is an idle timeout, well under an hour, that ends a session nobody has touched. The other is an absolute cap of 3600 seconds from login, which no amount of client-side activity extends; when it runs out you have to log in again. `status` prints that deadline as `expiresAt`. Run `keep-alive` yourself when you want a different interval or duration than the hour login starts.
...
PDF commands write a new file, mode 0600, and refuse to overwrite. An `--out` path that already exists fails with `OUTPUT_FILE_EXISTS`, a missing parent directory with `OUTPUT_DIRECTORY_MISSING`, a directory with `OUTPUT_PATH_IS_DIRECTORY`, and a path the user cannot write with `OUTPUT_NOT_WRITABLE`. All four exit 1 and name the problem, rather than asking for a bug report. They accept no raw document URL, path, identity, or access signature. Core PDF methods cap the document at 2 MiB; the CLI does not bypass that bound.
...
Exit codes: 0 success, 1 command/storage/upstream failure, 2 invalid usage, 3 login required, interactive terminal required, a prompt cancelled with Ctrl-C (`LOGIN_CANCELLED`), or a login started but not finished. `status` without `--verify` returns 0 when signed out, because it successfully reports local state. Invalid options, dates, timestamps, and list bounds are rejected before storage or network access.

All commands except `mcp` accept `--no-input` and `--json`. With `--json` on failure, stdout is empty and stderr is one object of the form `{"error":{"code":"AUTH_REQUIRED","message":"…","exitCode":3}}`. Parse `code` and the exit status. Missing or expired sessions mean a new login: the user in their terminal, or `login --id` followed by `login --code`; do not retry the read in an agent loop. One exit-3 path still writes to stdout: `login --id` with several SMS numbers available, where the option list is a result rather than a failure. Without `--json`, stderr is safe text with the same error code. Unknown exceptions never expose their original message or stack.
...
**The whole chain has been run against the live viewer**, twice, on 2026-09-23: all five commands exit 0 when validated against a live member account. Thumbnail JPEGs had the expected magic number and trailer; every pixel buffer matched the computed length exactly, including imaging pixel length checked on 8-bit single-frame instances with 1 and 3 samples. Mutating the study, series or image UID each returns `OWNER_MISMATCH` with no file written. What that still leaves open:

- Only 8-bit single-frame imaging is evidenced, in 1-sample and 3-sample form. 16-bit and multi-frame images are computed by the same arithmetic and validated against the bytes returned, so a wrong assumption fails loudly instead of producing a garbled file, but no CT or MR study has ever been read.
...
- `appointments --reference REF` uses a reference from the future-appointment list to retrieve supported provider contacts and visit instructions. Populated lists/details are source-backed/offline tested. Validated HTTP(S) instruction links are returned without fetching them; no scheduling or cancellation occurs.
