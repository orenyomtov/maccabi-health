...
The directory is created mode `0700`. Every file is written mode `0600`, created at that mode rather than chmod'ed afterwards, and swapped in with `rename`, so no truncated or world-readable version is ever observable, even if the process dies mid-write. On load, a file whose mode has any group or other bits set prints a warning to stderr, except on Windows, where Node reports the same bits for every file and there is nothing to act on.

The temporary file each write swaps in is named with a fresh UUID, so concurrent writes never share a path. A failed write only ever unlinks the temporary file it created itself; it never touches the real file, so a previously saved session survives intact.
