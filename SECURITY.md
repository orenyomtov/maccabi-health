...
Include what you need to make the problem reproducible: affected version, transport (library, CLI or MCP), and the steps. **Never include real credentials, cookies, session files, ID numbers, or medical records.** Use invented values, and describe the shape of the data rather than pasting it.
...
Sessions are stored under your user config directory. `maccabi logout --all` deletes every local credential, login challenge, registered OAuth client and issued token. Nothing is revoked upstream by that command.
